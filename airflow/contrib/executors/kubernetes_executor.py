...
class KubeConfig:
    ...
    def _validate(self):
        if not self.dags_in_image and not self.dags_volume_claim and (not self.git_repo or not self.git_branch):
            raise AirflowConfigException(
                'In kubernetes mode the following must be set in the `kubernetes` '
                'config section: `dags_in_image` or `dags_volume_claim` or `git_repo and git_branch`')
