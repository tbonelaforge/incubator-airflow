...
class WorkerConfiguration(LoggingMixin):
    ...
    def _get_init_containers(self, volume_mounts):
        ...
        # If the dags are built into the iamge, or we're using volume claims to mount the dags, no init container is needed
        if self.kube_config.dags_in_image or self.kube_config.dags_volume_claim:
            return []
    ...
    def _get_image_pull_secrets(self):
        self.log.debug("INSIDE worker_configuration.py, the WorkerConfiguration._get_image_pull_secrets method, got called...")
        """Extracts any image pull secrets for fetching container(s)"""
        if not self.kube_config.image_pull_secrets:
            self.log.debug("INSIDE worker_configuration.py, the WorkerConfiguration._get_image_pull_secrets method, realized self.kube_config does not contain image_pull_secrets, about to return []")
            return []
        self.log.debug("INSIDE worker_configuration.py, the WorkerConfiguration._get_image_pull_secrets method, about to return self.kube_config.image_pull_secrets.split(',')...")
        self.log.debug(self.kube_config.image_pull_secrets.split(','))
    ...
    def make_pod(self, namespace, worker_uuid, pod_id, dag_id, task_id, execution_date,
        ...
        self.log.debug("INSIDE worker_configuration.py, WorkerConfiguration.make_pod, about to construct a Pod object, with image_pull_secrets=")
        self.log.debug(self.kube_config.image_pull_secrets)
        return Pod(
            namespace=namespace,
            name=pod_id,
